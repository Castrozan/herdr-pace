...
from herdr_pace.reading.word import ReadingBreak, ReadingWord

from .actions import ReaderAction
from .word import has_trailing_punctuation
...
class ReaderPlayback:
    ...
    def apply(self, action: ReaderAction) -> None:
        if action == ReaderAction.TOGGLE:
            if self.finished:
                self.position = 0
        ...
        elif action == ReaderAction.RESTART:
            self.position = 0
            self.start_countdown()
        elif action in (ReaderAction.FASTER, ReaderAction.SLOWER):
            direction = 1 if action == ReaderAction.FASTER else -1
            self.words_per_minute = max(
                MIN_WPM, min(MAX_WPM, self.words_per_minute + direction * WPM_STEP)
            )
        else:
            self.adjust_countdown(1 if action == ReaderAction.LONGER_COUNTDOWN else -1)
